...
// GetContainersPlans - List containers plans
// Lists all containers plans.
func (s *Plans) GetContainersPlans(ctx context.Context, request operations.GetContainersPlansRequest, opts ...operations.Option) (*operations.GetContainersPlansResponse, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionRetries,
		operations.SupportedOptionTimeout,
	...
	opURL, err := url.JoinPath(baseURL, "/plans/containers")
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	hookCtx := hooks.HookContext{
		...
		BaseURL:          baseURL,
		Context:          ctx,
		OperationID:      "get-containers-plans",
		OAuth2Scopes:     []string{},
		SecuritySource:   s.sdkConfiguration.Security,
	}
	...
	res := &operations.GetContainersPlansResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/vnd.api+json`):
			...
			var out operations.GetContainersPlansResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
...
func (s *Plans) GetContainersPlan(ctx context.Context, planID string, opts ...operations.Option) (*operations.GetContainersPlanResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/plans/containers/{plan_id}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.GetContainersPlanResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/vnd.api+json`):
			...
			var out components.ContainerPlanData
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.ContainerPlanData = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
