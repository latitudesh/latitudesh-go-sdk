...
```go
latitudeshgosdk.WithServerURL("https://api.example.com")
```
...
```go
latitudeshgosdk.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
latitudeshgosdk.WithLatitudeAPIKey(/* ... */)
```
...
```go
latitudeshgosdk.WithSecuritySource(/* ... */)
```
...
```go
latitudeshgosdk.WithTimeout(30 * time.Second)
```
