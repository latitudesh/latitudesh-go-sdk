...
func newTeams(rootSDK *Latitudesh, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Teams {
	return &Teams{
		rootSDK:          rootSDK,
...
func (s *Teams) Get(ctx context.Context, opts ...operations.Option) (*operations.GetTeamResponse, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/team")
	if err != nil {
...
func (s *Teams) Create(ctx context.Context, request operations.PostTeamTeamsRequestBody, opts ...operations.Option) (*operations.PostTeamResponse, error) {
	...
	if retryConfig != nil {
		...
		if err != nil || httpRes == nil {
			...
			_, err = s.hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, nil, err)
			return nil, err
		} else if utils.MatchStatusCodes([]string{"406", "422", "4XX", "5XX"}, httpRes.StatusCode) {
			_httpRes, err := s.hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, httpRes, nil)
			if err != nil {
				return nil, err
			} else if _httpRes != nil {
	...
	case httpRes.StatusCode == 201:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/vnd.api+json`):
			...
			var out operations.PostTeamResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
...
func (s *Teams) Update(ctx context.Context, teamID string, requestBody operations.PatchCurrentTeamTeamsRequestBody, opts ...operations.Option) (*operations.PatchCurrentTeamResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/team/{team_id}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/vnd.api+json`):
			...
			var out operations.PatchCurrentTeamResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
