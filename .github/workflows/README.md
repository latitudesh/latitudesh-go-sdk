...
### 2. PR Tests & Coverage (`sdk_pr_tests.yaml`)

Specialized workflow for Pull Requests that adds automatic comments with results.
...
- Calculates test and coverage statistics
- Posts/updates comment on PR with:
  - Overall status (✅/❌)
  - Number of tests (total, passed, failed)
  - Coverage percentage
...
### 3. SDK Generation (`sdk_generation.yaml`)
...
### 4. SDK Publish (`sdk_publish.yaml`)
